    ...
    <div class="flex-1 flex flex-col bg-white overflow-hidden">
        ...
        <template v-else>
            ...
            <div class="px-6 py-4 flex-1 overflow-y-scroll">
                <!-- A message -->
                <chat-message v-for="(message, index) in activeChannel.messages" :key="index" :message="message"></chat-message>
            </div>
            <div class="pb-6 px-4 flex-none">
                <div class="flex rounded-lg border-2 border-grey overflow-hidden">
                    ...
                    <input ref="input" type="text" class="w-full px-4" placeholder="Message #general" @keyup.enter="placeMessage"/>
                </div>
            </div>
        </template>
    </div>
